...
# نمط البحث لاستثناء العينات الخاصة (Private Sample)
...
def load_data(file_path='Data 2025.xlsx'):
    ...
    # استثناء العينات الخاصة (Private Sample) من جميع الأعمدة
    before_cat = len(df)
    for col in ['فئة_العينة', 'اسم_العينة', 'اسم_المنشأة']:
        if col in df.columns:
            df[col] = df[col].astype(str).str.strip()
            df = df[~df[col].str.contains(PRIVATE_SAMPLE_PATTERN, na=False, regex=True)]
    excluded_cat = before_cat - len(df)
    if excluded_cat > 0:
        print(f'تم استبعاد {excluded_cat} صف من العينات الخاصة (Private Sample)')
